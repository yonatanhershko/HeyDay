...
const getResponsiveDimensions = (width, height) => {
  const { isTablet, isDesktop, isLargeDesktop, isMobile } = getResponsiveInfo(width);
...
export const getTheme = (isDark = false, width, height, isMobile) => {
  const dimensions = width && height ? getResponsiveDimensions(width, height) : {};
